...
function subscribeWithMailerLite(action,email){
  return new Promise((resolve,reject)=>{
    const callback='mlWebformSubmitted';
    const previousCallback=window[callback];
    const script=document.createElement('script');
    const timeout=setTimeout(()=>finish(new Error('Subscription timed out')),12000);
    const finish=(error,data)=>{
      clearTimeout(timeout);
      script.remove();
      if(previousCallback===undefined) delete window[callback];
      else window[callback]=previousCallback;
      if(error) reject(error);
      else resolve(data);
    };
    window[callback]=data=>{
      if(data?.success || data?.fieldsToConfirm?.length) finish(null,data);
      else finish(new Error(data?.message || 'Subscription failed'));
    };
    script.onerror=()=>finish(new Error('Subscription request failed'));
    const url=new URL(action);
    url.searchParams.set('fields[email]',email);
    url.searchParams.set('ml-submit','1');
    url.searchParams.set('anticsrf','true');
    url.searchParams.set('ajax','1');
    url.searchParams.set('guid',crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`);
...
$('#leadForm').addEventListener('submit',async e=>{
  ...
  status.textContent='';
  try{
    await subscribeWithMailerLite(form.action,$('#email').value.trim());
    form.reset();
